...
<div class="card__body">
    <div style="color: ${
      data.status == true ? data.thingColor.colorOn : data.thingColor.colorOff
    };" data-color-on="${data.thingColor.colorOn}" data-color-off="${
    data.thingColor.colorOff
  }" data-icon="${
    data.icon.icon != undefined ? data.icon.icon : ""
  }" class="card__body__icon pointer icons-size ${
    data.icon.icon != undefined ? data.icon.icon : ""
  } id="card-icon">
        ${data.thing == "input" ? getInput(data) : ""}${
    data.subfije != "" ? data.subfije : ""
  }
    </div>
    <h2 class="card__body__description pointer" title="Description: ${
      data.description
    }" id="card-description">${data.description}</h2>
    </div>
<footer class="card__footer">
    ...
    data.topic.subscribe.topic == "" ? "no topic" : data.topic.subscribe.topic
    ...
    data.topic.publish.topic == "" ? "no topic" : data.topic.publish.topic
  }" id="card-topic-publish" value="${data.topic.publish.topic}"/> 
    </div>
    <div class="card__footer__btn"><label class="switch"><input type="checkbox" ${
      data.status == true ? "checked" : ""
    } id="card-btn-status" /><span class="slider round"></span></label></div>
</footer>
`;
};
